...
public class CreateRackInputPanel extends JPanel {
    public CreateRackInputPanel(String titleString, String abortButtonName, String continueButtonName, Runnable onAbort, Consumer<Rack> onContinue) {
        ...
        JFormattedTextField capacity = new JFormattedTextField(new DefaultFormatterFactory(new NumberFormatter(NumberFormat.getIntegerInstance())), 0);
        ...
        textPanel.add(x);
        textPanel.add(new JLabel("Kapazität:"));
        textPanel.add(capacity);
        ...
        continueButton.addActionListener(actionEvent -> onContinue.accept(new Rack(Integer.parseInt(x.getText().replace(".", "")), Integer.parseInt(x.getText().replace(".", "")), Integer.parseInt(capacity.getText().replace(".", "")), (Temperature) temperatur.getSelectedItem())));
